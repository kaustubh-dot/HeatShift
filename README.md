...
| Canonical saved hashes | `fcb3…0d8a`, `de952…9420`, `88b9…f651` | `backend/heatshift/fixtures/saved/manifest.json` |
...
The bundled scenario and policy are versioned inputs. Saved outputs are generated by `backend.heatshift.cli generate-saved` with a separate 30-second offline evidence budget; the manifest records that budget, the fixture version, Python and OR-Tools versions, solver seed `7`, one search worker, canonical input hashes, and canonical output hashes. Only `wall_time_seconds` is excluded from canonical output hashing. The frontend accepts a saved solve only when the complete submitted scenario and policy match the bundle structurally and the heat adjustment matches; diagnosis also requires the exact job. Live mode remains the default; fallback is explicitly disclosed.
